...
export default function TripTransportationOutbound(){
    ...
    },[localStorage.getItem("transportTicketsOutbound")])

    const[loadingMessage, setLoadingMessage] = useState("Loading Outbound Route Data");
    ...
    function handlePrevious(){
        localStorage.setItem("transportTicketsOutbound", "")
    ...
    useEffect(() => {
        ...
        let departureTime = localStorage.getItem("returnTime");
        let ticketQuantity = localStorage.getItem("totalTicketQuantity");
        TripService.getReturnRouteData(castleId,date,departureTime,ticketQuantity).then((res) => {
            let result = res.data;
            console.log(res)
            if (result.code === 200){
        ...
        if(localStorage.getItem("transportTicketsOutbound")){
            setTransportTicketsBack(JSON.parse(localStorage.getItem("transportTicketsOutbound")));
        }
    },[])
    ...
    const[transportTicketsBack, setTransportTicketsBack] = useState([]);
    ...
    return(
        <section className="trip-section">
            ...
            <h1 className="heading">Suggested Route (Outbound)</h1>
            ...
            {ticketArray.length !== 0 ?
                ...
                <div className="confirm-tickets">
                    <h2>Outbound Tickets basket:</h2>
                    <h2 className="heading">Your Outbound Transportation ticket basket is currently Empty</h2>
                </div>
            }

            <div>
                <button onClick={handlePrevious} className="btn cancel-btn">previous</button>
            ...
            <TripTransportationDetails
                ...
                transportTickets = {transportTicketsBack}
